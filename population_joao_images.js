...
var {points} = require('/home/estathop/Desktop/MindSpaces/puc1code/joao_data_final_visual_analysis_and_imageability.json');
const { v4: uuidv4 } = require('uuid')


//console.log(points[0]['id'])
fs.writeFileSync('images_joao.ttl', '@prefix : <http://www.semanticweb.org/estathop/ontologies/2021/9/untitled-ontology-17#> . \n@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .\n@prefix owl: <http://www.w3.org/2002/07/owl#> . \n@prefix geo: <http://www.opengis.net/ont/geosparql#> . \n', function (err) {
  ...
  let textuuid = uuidv4();
    ...
    if (element.Textual_Analysis_Body.ta.dbpedia_linking.nodes.length>0){
      ...
      element.Textual_Analysis_Body.ta.dbpedia_linking.nodes.forEach(element =>{
        ...
        fs.appendFileSync('images_joao.ttl', '<'+element.metadata.uri+'> rdf:type owl:NamedIndividual , \n :URI ; \n :hasConfidence '+element.metadata.confidence+' . \n', function (err) {
                if (err) return console.log(err);
                console.log('something with files went wrong !');
              });
      })
...
*/
    ...
    fs.appendFileSync('images_joao.ttl', ':'+element.id.replace(/ /g,"_")+' rdf:type owl:NamedIndividual , \n :Image ; \n :hasArousal :arousal'+element.id.replace(/ /g,"_")+' ; \n ')
      ...
      //fs.appendFileSync('images_joao.ttl', nerstring+'\n')

      fs.appendFileSync('images_joao.ttl', ':hasText :'+textuuid+' ; \n')
      fs.appendFileSync('images_joao.ttl', ':hasValence :valence'+element.id.replace(/ /g,"_")+' ; \n')
      ...
      fs.appendFileSync('images_joao.ttl', semsegstring+'\n')
      ...
      fs.appendFileSync('images_joao.ttl', vergestring+'\n')}
      
      fs.appendFileSync('images_joao.ttl', ':hasDirection "'+element.direction+'" ; \n \
      ...
      ')
       ...
       //insert the line below to the string above in images66
       //:hasTextImageability "'+element.Textual_Analysis_Body.ta.imageability.metadata.text_imageability+'" ; \n \

      fs.appendFileSync('images_joao.ttl', ':arousal'+element.id.replace(/ /g,"_")+' rdf:type owl:NamedIndividual , \n \
      :Arousal ; \n \
...
/*
      element.Textual_Analysis_Body.ta.ner.nodes.forEach( (element2, index) => {
      fs.appendFileSync('images_joao.ttl', ':ner'+(index+1)+''+element.id.replace(/ /g,"_")+' rdf:type owl:NamedIndividual  , \n \
      :Ner ; \n\
      ...
      try{ element.Textual_Analysis_Body.ta.imageability.nodes.forEach(variablelabel => {
        if(element2.label === variablelabel.label){
          logical_counter1++;
          logical_counter2++;
          fs.appendFileSync('images_joao.ttl', ' ; \n \
 ...
 */
      element.Visual_Analysis_Body.segmentation.segm_labels.forEach((element3, index2) => {
        fs.appendFileSync('images_joao.ttl', ':semsegtop'+(index2+1)+'class'+element.id.replace(/ /g,"_")+' rdf:type owl:NamedIndividual , \n \
        :SemSegContainer ; \n \
...
/*
      element.Textual_Analysis_Body.ta.emotion_detection.nodes.forEach((element6, index6) => {
        fs.appendFileSync('images_joao.ttl', ':sentence_'+(index6+1)+'_'+element.id.replace(/ /g,"_")+' rdf:type owl:NamedIndividual , \n \
        :Sentence ; \n \
...
*/
      fs.appendFileSync('images_joao.ttl', ':'+textuuid+' rdf:type owl:NamedIndividual , \n \
      :Text ; \n \
      ...
      fs.appendFileSync('images_joao.ttl', sentencestring+' \n')*/

      fs.appendFileSync('images_joao.ttl', ':valence'+element.id.replace(/ /g,"_")+' rdf:type owl:NamedIndividual , \n \
      :Valence ; \n \
      ...
      element.Visual_Analysis_Body.classification.verge_labels.forEach((element78, index88 ) => {
        fs.appendFileSync('images_joao.ttl', ':vergetop'+(index88+1)+'class'+element.id.replace(/ /g,"_")+'  rdf:type owl:NamedIndividual , \n \
        :VergeContainer ; \n \
